...
impl TypeCheckModule for ShorthandMul {
    fn typecheck(&mut self, meta: &mut ParserMetadata) -> SyntaxResult {
        ...
        if self.tok.is_some() {
            ...
            shorthand_typecheck_allowed_types(meta, "multiply", &self.kind, &self.expr, &[
                Type::Num,
                Type::Int,
            ])?;
        }
    ...
    fn translate(&self, meta: &mut TranslateMetadata) -> FragmentKind {
        ...
        let expr = match self.kind {
            Type::Int => {
                ...
                ArithmeticFragment::new(var.to_frag(), ArithOp::Mul, expr).to_frag()
            }
            Type::Num => {
                let expr = self.expr.translate_eval(meta, self.is_ref);
                translate_computation_eval(meta, ArithOp::Mul, Some(var.to_frag()), Some(expr), self.is_ref)
            }
            _ => unreachable!("Unsupported type {} in shorthand multiplication operation", self.kind)
        };
