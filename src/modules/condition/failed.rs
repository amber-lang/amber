    ...
    fn parse(&mut self, meta: &mut ParserMetadata) -> SyntaxResult {
        ...
        if token(meta, "?").is_ok() {
            if !meta.context.is_fun_ctx && !meta.context.is_main_ctx && !meta.context.is_unsafe_ctx
            {
                return error!(
                    meta,
                    tok, "The '?' operator can only be used in the main block or function body"
                );
            }
                ...
                Ok(_) => match token(meta, "{") {
                    ...
                    Err(_) => match token(meta, ":") {
                        ...
                        Err(_) => {
                            return error!(
                                meta,
                                tok, "Failed expression must be followed by a block or statement"
                            )
                        }
                    },
                },
    ...
    fn translate(&self, meta: &mut TranslateMetadata) -> String {
        if self.is_parsed {
            ...
            if self.is_question_mark {
                ...
                ["__AS=$?;", "if [ $__AS != 0 ]; then", &block, "fi"].join("\n")
            }
        } else {
            String::new()
        }
