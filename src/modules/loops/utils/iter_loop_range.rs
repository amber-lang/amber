...
pub trait IterLoopRange {
    ...
    fn translate_range_loop(&self, range: &Range, meta: &mut TranslateMetadata) -> FragmentKind;
...
impl IterLoopRange for IterLoop {
    fn translate_range_loop(&self, range: &Range, meta: &mut TranslateMetadata) -> FragmentKind {
        ...
        if let (Some(from_val), Some(to_val)) =
            (range.from.get_integer_value(), range.to.get_integer_value())
        {
        ...
        let id = self
            .iter_global_id
            .expect("No global ID set for loop iterator");
