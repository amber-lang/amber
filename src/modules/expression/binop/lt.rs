    ...
    fn translate(&self, meta: &mut TranslateMetadata) -> FragmentKind {
        match self.left.get_type() {
            ...
            Type::Array(inner_type) => match *inner_type {
                Type::Int => {
                    translate_array_lexical_comparison(meta, ComparisonOperator::Lt, &self.left, &self.right, Type::Int)
                }
                Type::Text => {
                    translate_array_lexical_comparison(meta, ComparisonOperator::Lt, &self.left, &self.right, Type::Text)
                }
                _ => unreachable!("Unsupported array type {} in less than comparison", self.left.get_type())
            }
            Type::Text => {
