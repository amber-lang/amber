...
pub trait BinOp: SyntaxModule<ParserMetadata> {
    ...
    fn typecheck_allowed_types(
        ...
        if !left_match || !right_match {
            ...
            let msg = format!("Cannot perform {operator} on value of type '{left_type}' and value of type '{right_type}'");
    ...
    fn typecheck_equality(
        ...
        if left_type != right_type {
            let pos = get_binop_position_info(meta, left, right);
            let message = Message::new_err_at_position(meta, pos);
            let msg = format!("Expected both operands to be of the same type, but got '{left_type}' and '{right_type}'.");
            Err(Failure::Loud(message.message(msg)))
        } else {
            Ok(left_type)
        }
    }
