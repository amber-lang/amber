...
pub fn handle_add_function(meta: &mut ParserMetadata, name: &str, args: &[(String, Type)], returns: Type, tok: Option<Token>, body: Vec<Token>) {
    ...
    meta.mem.add_function(name, args, returns, body);
}

pub fn run_function_with_args(meta: &mut ParserMetadata, name: &str, args: &[Type]) {
...
pub fn handle_function_parameters(meta: &mut ParserMetadata, name: &str, args: &[Type]) -> Type {
    let function_unit = meta.mem.get_function(&name).unwrap().clone();
    // TODO: Here is a good place to insert trace
    run_function_with_args(meta, name, args);
    function_unit.returns.clone()
}
