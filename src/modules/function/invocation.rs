    ...
    fn parse(&mut self, meta: &mut ParserMetadata) -> SyntaxResult {
        ...
        self.modifier.use_modifiers(meta, |_this, meta| {
            ...
            (self.kind, self.variant_id) =
                handle_function_parameters(meta, self.id, function_unit, &types, &var_names, tok)?;
            Ok(())
        })
    }
}
...
impl FunctionInvocation {
    fn get_variable(&self, meta: &TranslateMetadata, name: &str, dollar_override: bool) -> String {
        let dollar = dollar_override
            .then_some("$")
            .unwrap_or_else(|| meta.gen_dollar());
...
impl TranslateModule for FunctionInvocation {
    fn translate(&self, meta: &mut TranslateMetadata) -> String {
        ...
        let invocation_instance = &format!(
            "__AF_{}{}_v{}__{}_{}",
            self.name, self.id, self.variant_id, self.line, self.col
        );
