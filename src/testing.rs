...
pub fn get_tests_to_run(command: &TestCommand) -> Result<Vec<(PathBuf, String, String)>, Vec<Message>> {
    let input_path = &command.input;
    let mut files = vec![];
    if input_path.is_dir() {
        find_amber_files(input_path, &mut files).map_err(|e| vec![Message::new_err_msg(e.to_string())])?;
    } else {
    ...
    for file in &files {
        let code = match fs::read_to_string(file) {
            Ok(c) => c,
            Err(e) => {
                errors.push(Message::new_err_msg(format!("Failed to read file {}: {}", file.display(), e)));
                continue;
            }
        };
